...
int LCS::LCSLength()
{
	//初始化行标或列标为0的元素，由于二维数组先行初始化为0过，不再需要
	...
	for (int i = 1; i <= m; i++)
		for (int j = 1; j <= n; j++)
			...
			{
				c[i][j] = c[i - 1][j - 1] + 1; 
				s[i][j] = 1; //s中存储计算过程，3表示矩阵计算中左＜上，1表示左=上，2表示左＞上
			}
			else
				if (c[i - 1][j] >= c[i][j - 1]) 
				{
					c[i][j] = c[i - 1][j]; 
					s[i][j] = 3; //注意：c[i - 1][j]表示上方元素
				}
				else 
				{
					c[i][j] = c[i][j - 1]; 
					s[i][j] = 2; //注意：c[i][j - 1]表示左方元素
				}
	return c[m][n]; //返回最优解值
}
...
void LCS::CLCS(int i, int j) const
{
	...
	if (s[i][j] == 1)  //即：a[i] == b[j]
	{
		...
		if (s[i][j] == 3) CLCS(i - 1, j);   //c[i - 1][j] >= c[i][j - 1]
		else CLCS(i, j - 1);  //c[i - 1][j] < c[i][j - 1]
}
void LCS::CLCS() {
	CLCS(m, n);
}
